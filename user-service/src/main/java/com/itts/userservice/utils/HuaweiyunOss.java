package com.itts.userservice.utils;
...
public class HuaweiyunOss {
    ...
    @Value("${huawei.obs.ak}")//ak
    private String ak;
    @Value("${huawei.obs.sk}")//sk
    private String sk;
    @Value("${huawei.obs.bucketName}")//桶名称
    private String bucketName;
    @Value("${huawei.obs.upload.endpoint}")//终端节点访问
     ...
     * @return
     */
    public boolean putFilebyInstreamAndMeta(
            String fileName,String string, InputStream inputStream) {
        ObsClient obsClient = null;
        try {
            String objectName = string.concat("/").concat(fileName);
            obsClient = new ObsClient(ak, sk, endpoint);
            HeaderResponse response = obsClient.putObject(bucketName, objectName, inputStream);
            // 可选：调用成功后，记录调用成功的HTTP状态码和服务端请求ID
            int statusCode = response.getStatusCode();
            if (200 == statusCode) {
                return true;
