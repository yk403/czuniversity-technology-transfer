 ...
 */
@RestController
@RequestMapping(BASE_URL+"/v1/LyRy")
@Api(value = "LyRyController", tags = "双创路演人员门户")
public class LyRyController {
    @Autowired
    private LyRyService lyRyService;
//    *
//     * 获取列表
...
//return null;
        return ResponseUtil.success(lyRyService.findLyRyFront(params));
    }
//    *
//     * 根据ID查询
//     * @param id
//     * @return
    ...
    public ResponseUtil getById(@PathVariable("id") Long id) {
        return ResponseUtil.success(lyRyService.getById(id));
    }
//    *
//     * 保存
    ...
    public ResponseUtil update(@RequestBody LyRy lyRy) throws WebException {
        ...
        //检查数据库中是否存在要更新的数据
        if (lyRyService.getById(id) == null) {
            throw new WebException(SYSTEM_NOT_FIND_ERROR);
        }
        //更新数据
        if (!lyRyService.updateById(lyRy)) {
    ...
    public ResponseUtil delete(@PathVariable("id") Long id) {
        ...
        LyRy old = lyRyService.getById(id);
        if (old == null) {
