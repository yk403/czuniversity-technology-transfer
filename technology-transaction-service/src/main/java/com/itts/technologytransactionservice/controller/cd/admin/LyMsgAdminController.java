 ...
 */
@RestController
@RequestMapping(ADMIN_BASE_URL+"/v1/LyMsg")
@Api(value = "LyMsgAdminController", tags = "双创路演留言后台管理")
public class LyMsgAdminController {
    @Autowired
    private LyMsgAdminService lyMsgAdminService;
//    *
//     * 获取列表
...
//return null;
        return ResponseUtil.success(lyMsgAdminService.findLyMsgBack(params));
    }
//    *
//     * 根据ID查询
//     * @param id
//     * @return
    ...
    public ResponseUtil getById(@PathVariable("id") Long id) {
        return ResponseUtil.success(lyMsgAdminService.getById(id));
    }
//    *
//     * 保存
    ...
    public ResponseUtil save(@RequestBody LyMsg lyMsg) {
        if (!lyMsgAdminService.saveMsg(lyMsg)) {
    ...
    public ResponseUtil update(@RequestBody LyMsg lyMsg) throws WebException {
        ...
        //检查数据库中是否存在要更新的数据
        if (lyMsgAdminService.getById(id) == null) {
    ...
    public ResponseUtil delete(@PathVariable("id") Long id) {
        ...
        LyMsg old = lyMsgAdminService.getById(id);
        if (old == null) {
        ...
        lyMsgAdminService.updateById(old);
