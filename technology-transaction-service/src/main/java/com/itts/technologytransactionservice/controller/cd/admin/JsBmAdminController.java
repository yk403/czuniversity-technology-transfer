    ...
    */
    ...
    public R update(@RequestBody TJsBm tJsBm) {
        ...
        if(tJsBm.getShzt().equals("2")){
            jsXtxxService.addXtxx(jsXtxxService.getUserId(),byId.getUserId()==null?null:byId.getUserId().longValue(),6,0,jsHdAdminService.getById(byId.getHdId()).getHdmc());
        }
        if(tJsBm.getShzt().equals("1")){
            jsXtxxService.addXtxx(jsXtxxService.getUserId(),byId.getUserId()==null?null:byId.getUserId().longValue(),6,1,jsHdAdminService.getById(byId.getHdId()).getHdmc());
        }
        return update(true);
    }
