...
public class JsShAdminServiceImpl extends ServiceImpl<JsShMapper, TJsSh> implements JsShAdminService {
    ...
    public Boolean auditCg(Map<String, Object> params, Integer fbshzt) {
        ...
        if(fbshzt == 2){
             ...
             * @Date: 2021/7/6
             */
            jsXtxxService.addXtxx(jsXtxxService.getUserId(),cg.getUserId().longValue(),0,"信息采集审核管理","技术成果","审核成功");
        }else if(fbshzt == 4){
            TJsCg cg=jsCgMapper.getById(Integer.parseInt(params.get("id").toString()));
            /**
             * @Description: 发送人员id,接收人员id，模块类型(0:技术交易,1:双创路演)，所属模块(信息采集，报名，招标发布，挂牌发布等)，载体类型(如技术成果，技术需求，技术展品)，操作内容（如发布成功，发布失败，审核成功等）
             ...
             * @Date: 2021/7/6
             */
            jsXtxxService.addXtxx(jsXtxxService.getUserId(),cg.getUserId().longValue(),0,"信息采集审核管理","技术成果","审核拒绝");
        }
        return true;
    }
    ...
    public Boolean auditXq(Map<String, Object> params, Integer fbshzt) {
        ...
        if(fbshzt == 2){
             ...
             * @Date: 2021/7/6
             */
            jsXtxxService.addXtxx(jsXtxxService.getUserId(),xq.getUserId().longValue(),0,"信息采集审核管理","技术需求","审核成功");
        }else if(fbshzt == 4){
            TJsXq xq=jsXqMapper.getById(Integer.parseInt(params.get("id").toString()));
            /**
             * @Description: 发送人员id,接收人员id，模块类型(0:技术交易,1:双创路演)，所属模块(信息采集，报名，招标发布，挂牌发布等)，载体类型(如技术成果，技术需求，技术展品)，操作内容（如发布成功，发布失败，审核成功等）
             ...
             * @Date: 2021/7/6
             */
            jsXtxxService.addXtxx(jsXtxxService.getUserId(),xq.getUserId().longValue(),0,"信息采集审核管理","技术需求","审核拒绝");
        }
        return true;
    }
    ...
    public Boolean assistanceAuditXq(Map<String, Object> params, Integer assistanceStatus) {
        ...
        if(assistanceStatus == 2){
             ...
             * @Date: 2021/7/6
             */
            jsXtxxService.addXtxx(jsXtxxService.getUserId(),xq.getUserId().longValue(),0,"技术招标审批管理","技术需求","审核成功");
             ...
             * @Date: 2021/7/6
             */
            jsXtxxService.addXtxx(jsXtxxService.getUserId(),xq.getUserId().longValue(),0,"技术招标审批管理","技术需求","审核拒绝");
        }
        return true;
    }
    ...
    public Boolean assistanceAuditCg(Map<String, Object> params, Integer assistanceStatus) {
        ...
        if(assistanceStatus == 2){
             ...
             */
            if(tJsSh.getJylx() == 0){
                jsXtxxService.addXtxx(jsXtxxService.getUserId(),cg.getUserId().longValue(),0,"技术拍卖审核管理","技术成果","审核成功");
            }
            if(tJsSh.getJylx() == 2){
                jsXtxxService.addXtxx(jsXtxxService.getUserId(),cg.getUserId().longValue(),0,"技术挂牌审核管理","技术成果","审核成功");
            }
        }else if(assistanceStatus == 4){
            TJsCg cg=jsCgMapper.getById(Integer.parseInt(params.get("id").toString()));
            /**
             ...
             */
            if(tJsSh.getJylx() == 0){
                jsXtxxService.addXtxx(jsXtxxService.getUserId(),cg.getUserId().longValue(),0,"技术拍卖审核管理","技术成果","审核拒绝");
            }
            if(tJsSh.getJylx() == 2){
                jsXtxxService.addXtxx(jsXtxxService.getUserId(),cg.getUserId().longValue(),0,"技术挂牌审核管理","技术成果","审核拒绝");
            }
        }
        return true;
    }
